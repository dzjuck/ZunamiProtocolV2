...
describe('Conic Oracle', async () => {
    ...
    describe('Deployment', async () => {
        it('Should correctly deploy the contracts', async () => {
            // given
            const { owner, curveLPOracle, genericOracle } = await loadFixture(deployFixture);
    ...
    describe('CurveRegistryCache', async () => {
        it('Should init two pools in registry', async () => {
            // given
            const { curveRegistryCache } = await loadFixture(deployFixture);
            ...
            expect(
                (await curveRegistryCache.lpToken(crvUSD_USDT_pool_addr)).toLowerCase()
            ...
            expect(
                (await curveRegistryCache.lpToken(crvUSD_USDC_pool_addr)).toLowerCase()
            ).to.equal(crvUSD_USDC_pool_addr.toLowerCase());

            expect(await curveRegistryCache.isRegistered(stETH_ETH_pool_addr)).to.equal(true);
            expect((await curveRegistryCache.lpToken(stETH_ETH_pool_addr)).toLowerCase()).to.equal(
                stETH_ETH_pool_addr.toLowerCase()
            );
        });
    });
    describe('CurveLPOracle', async () => {
        ...
        it('Should return LP prices', async () => {
            const { curveLPOracle, curveRegistryCache, chainlinkOracle } = await loadFixture(
                deployFixture
            );
            await curveRegistryCache.initPool(crvUSD_USDT_pool_addr);
            await curveRegistryCache.initPool(crvUSD_USDC_pool_addr);
            ...
            expect(
                isInRange(
                    crvUSD_USDT_price,
                    crvUSD_USDT_virtualPrice.sub(stableThreshold),
                    crvUSD_USDT_virtualPrice.add(stableThreshold)
                )
            ).to.equal(true);
            expect(
                isInRange(
                    crvUSD_USDC_price,
                    crvUSD_USDC_virtualPrice.sub(stableThreshold),
                    crvUSD_USDC_virtualPrice.add(stableThreshold)
                )
            ).to.equal(true);
            expect(
                isInRange(
                    stETH_ETH_price.div(stETH_usdPrice).mul(BigNumber.from(10).pow(18)),
