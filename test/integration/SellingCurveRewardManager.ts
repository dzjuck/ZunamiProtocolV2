...
describe('SellingCurveRewardManager', async () => {
    ...
    async function deployFixture() {
        ...
        const rewardManager = (await SellingCurveRewardManager.deploy(converter.address)) as SellingCurveRewardManager;
  ...
  it('should correctly valuate rewards in stables', async () => {
    ...
    for (const reward of rewards) {
      for (const token of tokens) {
        expect(await rewardManager.valuate(reward, amount.toString(), token)).greaterThan(0);
      }
    }
  });
