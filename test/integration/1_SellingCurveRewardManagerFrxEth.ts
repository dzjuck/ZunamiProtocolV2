...
import { SellingCurveRewardManagerFrxEth } from '../../typechain-types';
...
describe.skip('1_SellingCurveRewardManagerFrxEth', async () => {
    ...
    async function deployFixture() {
        ...
        const SellingCurveRewardManagerFrxEth = await ethers.getContractFactory(
            'SellingCurveRewardManagerFrxEth'
        );
        const rewardManager = (await SellingCurveRewardManagerFrxEth.deploy(
            converter.address
        )) as SellingCurveRewardManagerFrxEth;
    ...
    it('should correctly valuate rewards in frxETH', async () => {
        ...
        for (const reward of rewards) {
            expect(await rewardManager.valuate(reward, amount.toString(), frxEthAddr)).greaterThan(
                0
            );
        }
    });
    ...
    it('should correctly sell rewards to frxETH', async () => {
        ...
        for (let i = 0; i < rewards.length; i++) {
            ...
            await mintTokenTo(rewardManager.address, owner, rewards[i], rewardVaults[i], amount);
