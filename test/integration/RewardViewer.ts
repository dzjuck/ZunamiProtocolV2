...
import {
  createConvertersAndRewardManagerContracts,
} from '../utils/CreateConvertersAndRewardManagerContracts';
...
describe('Reward Viewer', async () => {
  ...
  describe('Get convex-curve strategy rewards', async () => {
    it('Should return rewards for ETH_stETH strategy', async () => {
      // given
      const {owner, rewardViewer, otherAccount} = await loadFixture(
        deployFixture
      );
      ...
      const stEthEthConvexCurveStrategy =
        (await stEthEthConvexCurveStrategyFactory.deploy(
          genericOracle.address
      ...
      await wEth.connect(otherAccount)
        .approve(zunamiPoolController.address, parseUnits(tokenApprovedAmount, 'ether'));
      await frxEth.connect(otherAccount)
        .approve(zunamiPoolController.address, parseUnits(tokenApprovedAmount, 'ether'));
      const tokenAmount = '100';
      await wEth.transfer(otherAccount.getAddress(), ethers.utils.parseUnits(tokenAmount, 'ether'));
      await frxEth.transfer(otherAccount.getAddress(), ethers.utils.parseUnits(tokenAmount, 'ether'));
