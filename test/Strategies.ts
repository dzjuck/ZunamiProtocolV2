...
import {
  ChainlinkOracle, CurveLPOracle, CurveRegistryCache, GenericOracle,
  ZunamiPoolUZD, ZunamiPooControllerUZD, StableConverter, SellingCurveRewardManager
...
const curve_TricryptoUSDC_abi = [
  {"stateMutability":"payable","type":"function","name":"exchange_underlying","inputs":[{"name":"i","type":"uint256"},{"name":"j","type":"uint256"},{"name":"dx","type":"uint256"},{"name":"min_dy","type":"uint256"}],"outputs":[{"name":"","type":"uint256"}]},
...
const curve_3pool_abi = [{"name":"exchange","outputs":[],"inputs":[{"type":"int128","name":"i"},{"type":"int128","name":"j"},{"type":"uint256","name":"dx"},{"type":"uint256","name":"min_dy"}],"stateMutability":"nonpayable","type":"function"}];
...
describe('Single strategy tests', () => {
    ...
    async function deployFixture() {
      ...
      const curveTricryptoUSDC = new ethers.Contract(curve_TricryptoUSDC_addr, curve_TricryptoUSDC_abi, admin);
      ...
      const zunamiPoolController = await ZunamiPooControllerUZDFactory.deploy(zunamiPool.address) as ZunamiPooControllerUZD;
      ...
      const rewardManager = await SellingCurveRewardManagerFactory.deploy(stableConverter.address) as SellingCurveRewardManager;
      ...
      for (const user of [admin, alice, bob]) {
        await dai.connect(user).approve(zunamiPoolController.address, parseUnits('1000000', 'ether'));
        await usdc.connect(user).approve(zunamiPoolController.address, parseUnits('1000000', 'mwei'));
        await usdt.connect(user).approve(zunamiPoolController.address, parseUnits('1000000', 'mwei'));
      }
    ...
    it('should deposit assets', async () => {
      ...
      for (let poolId = 0; poolId < strategies.length; poolId++) {
          ...
          for (const user of [admin, alice, bob]) {
              ...
              await expect(zunamiPoolController.connect(user).deposit(getMinAmount(), await user.getAddress())).to.emit(
                  zunamiPool,
                  'Deposited'
              );
    ...
    it('should withdraw assets', async () => {
      ...
      for (let poolId = 0; poolId < strategies.length; poolId++) {
          ...
          for (const user of [alice, bob]) {
              await expect(zunamiPoolController.connect(user).deposit(getMinAmount(), user.getAddress())).to.emit(
                zunamiPool,
                  'Deposited'
              );
    ...
    it('should claim and withdraw all rewards', async () => {
        ...
        for (let i = 0; i < strategies.length; i++) {
            ...
            await expect(zunamiPoolController.connect(alice).deposit(getMinAmount(), admin.getAddress())).to.emit(
                zunamiPool,
                'Deposited'
            );
        }
    ...
    it.only('should moveFunds only to not outdated pool', async () => {
        ...
        await expect(zunamiPoolController.connect(alice).deposit(getMinAmount(), admin.getAddress())).to.emit(zunamiPool, 'Deposited');
        await expect((await zunamiPool.poolInfo(poolSrc)).deposited).to.be.gt(0);

        console.log("(await zunamiPool.poolInfo(poolSrc)).deposited", (await zunamiPool.poolInfo(poolSrc)).deposited);
        console.log("(await zunamiPool.poolInfo(poolDst)).deposited", (await zunamiPool.poolInfo(poolDst)).deposited);
        await expect((await zunamiPool.poolInfo(poolSrc)).deposited).to.be.gt(0);
