...
describe('RecapitalizationManager', () => {
  ...
  beforeEach(async () => {
    ...
    stakingRewardDistributor  = await smock.fake('IStakingRewardDistributor') as FakeContract<IStakingRewardDistributor>;

    const currentBlock = (await ethers.provider.getBlockNumber()) + 1;
    const RecapitalizationManager = await ethers.getContractFactory('RecapitalizationManager', admin);
    ...
    await expect(
      recapitalizationManager.setRewardTokens([])
    ).to.be.revertedWithCustomError(recapitalizationManager,
      `WrongRewardTokens`);
  ...
  it('should be created rightly', async () => {
    await expect(await recapitalizationManager.rewardTokens(0)).to.be.equal(rewardTokens[0].address);
    await expect(await recapitalizationManager.rewardTokens(1)).to.be.equal(rewardTokens[1].address);
    await expect(await recapitalizationManager.rewardTokens(2)).to.be.equal(rewardTokens[2].address);

    await expect(await recapitalizationManager.zunToken()).to.be.equal(zun.address);
    await expect(await recapitalizationManager.stakingRewardDistributor()).to.be.equal(ADDRESS_ZERO);
    const accumulationPeriod = await recapitalizationManager.ACCUMULATION_PERIOD();
    await expect(await recapitalizationManager.accumulationPeriod()).to.be.equal(accumulationPeriod.toString());
  });
  ...
  it('should be set staking reward distributor', async () => {
    ...
    await expect(await recapitalizationManager.stakingRewardDistributor()).to.be.equal(bob.address);
  });

  it('should be set accumulation period', async () => {
    await expect(await recapitalizationManager.accumulationPeriod()).to.be.equal(await recapitalizationManager.ACCUMULATION_PERIOD());

    const newAccumulationPeriod = (await recapitalizationManager.ACCUMULATION_PERIOD()).add((14 * 24 * 60 * 60) / 12);
    ...
    await expect(await recapitalizationManager.accumulationPeriod()).to.be.equal(newAccumulationPeriod);
  });
  ...
  it('should recapitalize pool by rewards', async () => {
    ...
    await expect(
      recapitalizationManager.connect(admin).recapitalizePoolByRewards(
    ...
    await recapitalizationManager.connect(admin).recapitalizePoolByRewards(
      rewardManager.address,
      zunamiPool.address,
      sid,
      tid
    ...
    zunamiPool.depositStrategy.atCall(0).should.be.calledWith(sid, [amount.toFixed(), 0, 0, 0, 0]);
  });
  ...
  it('should recapitalize pool by staked zun', async () => {
    ...
    await recapitalizationManager.connect(admin).setRewardDistributor(stakingRewardDistributor.address);
    ...
    await poolToken.transfer.whenCalledWith(zunamiPool.address, zunAmount.toFixed()).returns(true);

    await recapitalizationManager.connect(admin).recapitalizePoolByStackedZun(
      zunAmount.toString(),
      rewardManager.address,
      zunamiPool.address,
      sid,
    ...
    stakingRewardDistributor.withdrawPoolToken.atCall(0).should.be.calledWith(zun.address, zunAmount.toString());
    rewardManager.handle.atCall(0).should.be.calledWith(zun.address, zunAmount.toFixed(), poolToken.address);
    zunamiPool.depositStrategy.atCall(0).should.be.calledWith(sid, [zunAmount.toFixed(), 0, 0, 0, 0]);
  });
  ...
  it('should capitalize staked zun by rewards', async () => {
    ...
    await zun.approve.whenCalledWith(stakingRewardDistributor.address, zunAmount.toFixed()).returns(true);

    await recapitalizationManager.connect(admin).capitalizeStakedZunByRewards(rewardManager.address);

    stakingRewardDistributor.returnPoolToken.atCall(0).should.be.calledWith(zun.address, zunAmount.toFixed());
  });
});
