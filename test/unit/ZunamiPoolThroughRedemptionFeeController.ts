...
describe('ZunamiPoolThroughRedemptionFeeController', () => {
    ...
    it('should be set fee params', async () => {
        ...
        await expect(
            zunamiController
                .connect(admin)
                .changeWithdrawFee((await zunamiController.MAX_FEE()).add(1))
        ).to.be.revertedWithCustomError(zunamiController, `FeeWronglyHigh`);
        ...
        await expect(await zunamiController.withdrawFee()).to.be.equal(
            await zunamiController.MAX_FEE()
        );
    ...
    it('should withdraw user funds minus fee', async () => {
        ...
        const nominalFee = amount
            .multipliedBy(withdrawFee.toString())
            .dividedBy(feeDenominator.toString());

        await zunamiPool.transferFrom
            .whenCalledWith(admin.address, zunamiController.address, amount.toFixed())
            .returns(true);
        await zunamiPool.transfer
            .whenCalledWith(
                (await zunamiController.feeDistributor()).toString(),
                nominalFee.toFixed()
            )
            .returns(true);
        ...
        zunamiPool.withdraw
            ...
            .should.be.calledWith(
                sid,
                amount.minus(nominalFee).toFixed(),
                tokenBalances,
                alice.address
            );
    });
