...
import { IERC20, IPool, IRewardManager } from '../../typechain-types';
...
describe('ZunamiPoolCompoundController', () => {
    ...
    beforeEach(async () => {
        ...
        zunamiController = await ZunamiController.deploy(
            zunamiPool.address,
            'Zunami APS TEST',
            'zunAPSTEST'
        );
    ...
    it('should deposit user funds', async () => {
        ...
        await zunamiPool.deposit
            .whenCalledWith(sid, tokenBalances, zunamiController.address)
            .returns(amount.toFixed());
        ...
        await zunamiPool.deposit
            .whenCalledWith(sid, tokenBalances, zunamiController.address)
            .returns(amount.toFixed());
        await zunamiPool.balanceOf
            .whenCalledWith(zunamiController.address)
            .returns(amount.toFixed());
    ...
    it('should withdraw user funds', async () => {
        ...
        await zunamiPool.deposit
            .whenCalledWith(sid, tokenBalances, zunamiController.address)
            .returns(deposited.toFixed());
        await zunamiPool.balanceOf.whenCalledWith(zunamiController.address).returns(0);
        await zunamiController.deposit(tokenBalances, admin.address);

        expect(await zunamiController.balanceOf(admin.address)).to.be.equal(
            deposited.minus(MINIMUM_LIQUIDITY).toFixed()
        );
        ...
        expect(await zunamiController.balanceOf(admin.address)).to.be.equal(
            deposited.minus(amount).minus(MINIMUM_LIQUIDITY).toFixed()
        );
        await zunamiPool.balanceOf
            .whenCalledWith(zunamiController.address)
            .returns(deposited.minus(amount).toFixed());
        ...
        zunamiPool.withdraw
            .atCall(1)
            .should.be.calledWith(sid, remainder.toFixed(), tokenBalances, admin.address);
    ...
    it('should be do nothing on compounding if rewards are empty', async () => {
        ...
        await expect(zunamiController.connect(bob).autoCompoundAll()).to.be.revertedWithCustomError(
            zunamiController,
            `ZeroFeeTokenAddress`
        );
        ...
        zunamiPool.claimRewards
            .atCall(0)
            .should.be.calledWith(zunamiController.address, [
    ...
    it('should be compounded rewards', async () => {
        ...
        await zunamiPool.deposit
            ...
            .returns(deposited.toFixed());
        await zunamiPool.balanceOf.whenCalledWith(zunamiController.address).returns(0);
        await daiFake.transferFrom
            .whenCalledWith(admin.address, zunamiPool.address, tokenify(100).toFixed())
            .returns(true);
        await zunamiController.deposit(tokenBalances, admin.address);
        ...
        await daiFake.transfer
            .whenCalledWith(zunamiPool.address, tokenify(150 - 150 * 0.1).toFixed())
            .returns(true);
        await zunamiController.connect(bob).autoCompoundAll();

        expect(await zunamiController.collectedManagementFee()).to.be.equal(
            tokenify(150 * 0.1).toFixed()
        );
        ...
        for (let i = 0; i < rewardTokens.length; i++) {
            rewardManager.handle
                ...
                .should.be.calledWith(
                    rewardTokens[i].address,
                    tokenify(100).toFixed(),
                    daiFake.address
                );
        }
        ...
        await daiFake.transfer
            .whenCalledWith(admin.address, tokenify(150 * 0.1).toFixed())
            .returns(true);

        await expect(zunamiController.setFeeTokenId(1)).to.be.revertedWithCustomError(
            zunamiController,
            `FeeMustBeWithdrawn`
        );

        await expect(zunamiController.claimManagementFee())
            .to.be.emit(zunamiController, `ClaimedManagementFee`)
            .withArgs(daiFake.address, tokenify(150 * 0.1).toFixed());
    });

    it('should be set fee token', async () => {
        await expect(await zunamiController.feeTokenId()).to.be.equal(0);

        await expect(zunamiController.setFeeTokenId(3)).to.be.revertedWithCustomError(
            zunamiController,
            `WrongTokenId`
        );

        await expect(zunamiController.setFeeTokenId(5 + 1)).to.be.revertedWithCustomError(
            zunamiController,
            `WrongTokenId`
        );
    ...
    it('should be set fee distributor', async () => {
        ...
        ).to.be.revertedWithCustomError(zunamiController, `ZeroAddress`);
    ...
    it('should be set management fee', async () => {
        ...
        await expect(zunamiController.setManagementFeePercent(301)).to.be.revertedWithCustomError(
            zunamiController,
            `WrongFee`
        );
    ...
    it('should be calculate compound controller token price', async () => {
        ...
        await expect(zunamiController.tokenPrice()).to.be.reverted;
