...
describe('ZunamiPoolThroughController', () => {
  ...
  beforeEach(async () => {
    ...
    const ZunamiController = await ethers.getContractFactory('ZunamiPoolThroughController', admin);
  ...
  it('should be claimed rewards', async () => {
    ...
    zunamiPool.claimRewards.atCall(0).should.be.calledWith(admin.address, rewardTokens.map(t => t.address));
  });
  ...
  it('should withdraw user funds', async () => {
      ...
      zunamiPool.withdraw.atCall(1).should.be.calledWith(sid, amount, tokenBalances, admin.address);
    });
