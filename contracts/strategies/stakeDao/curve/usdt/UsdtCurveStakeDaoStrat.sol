...
contract UsdtCurveStakeDaoStrat is CurveStakeDaoStratBase {
    ...
    function convertLiquidityTokenAmount(uint256[5] memory amounts) internal view override returns(uint256[2] memory) {
        return calcPoolUsdtAmounts(amounts);
    }
    ...
    function convertAndApproveTokens(
        ...
        IERC20Metadata usdt = zunami.tokens()[ZUNAMI_USDT_TOKEN_ID];

        convertZunamiStableToUsdt(ZUNAMI_DAI_TOKEN_ID, amounts[ZUNAMI_DAI_TOKEN_ID]); // DAI
        convertZunamiStableToUsdt(ZUNAMI_USDC_TOKEN_ID, amounts[ZUNAMI_USDC_TOKEN_ID]); // USDC

        amounts2[CURVE_POOL_USDT_ID] = usdt.balanceOf(address(this));
    ...
    function getCurveRemovingTokenIndex() internal pure override returns(int128) {
        return CURVE_POOL_USDT_ID_INT;
    }

    function calcPoolUsdtAmounts(uint256[5] memory amounts) internal view returns(uint256[2] memory amounts2) {
        if(
